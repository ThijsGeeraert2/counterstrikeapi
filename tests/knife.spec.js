...
describe("Knife API", () => {
    it("GET /api/knives should list of more than 2 knives", async () => {
        const response = await request(app).get('/api/knives');
    ...
    it("GET /api/gloves return correct knife 6650806e5c2a2875501e8147", async () => {
        const idObject = "6650806e5c2a2875501e8147"
        const expectedObject = {
            _id: "6650806e5c2a2875501e8147",
        ...
        const response = await request(app).get(`/api/knives/${idObject}`);
        expect(response.status).toBe(200);
